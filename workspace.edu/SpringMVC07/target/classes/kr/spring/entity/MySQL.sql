...
INSERT INTO TBLBOARD
SELECT IFNULL(MAX(IDX + 1), 1),
'pbk', 'spring반 친구들은 들으라', '맛있는 간식 또 사줄게요^^.', '병관쌤',
NOW(), 0, IFNULL(MAX(BOARDGROUP + 1), 1), 0, 0, 1
FROM TBLBOARD;

INSERT INTO TBLBOARD
SELECT IFNULL(MAX(IDX + 1), 1),
'jjy', '준용쌤입니다! 여러분 제발 늦지마세요', '아침마다 연락 자주받는 친구들 알겠지?', '준용대디',
NOW(), 0, IFNULL(MAX(BOARDGROUP + 1), 1), 0, 0, 1
FROM TBLBOARD;

INSERT INTO TBLMEMBER(MEMID, MEMPW, MEMNAME, MEMPHONE)
VALUES ('aishool', '1234', '교육운영부', '010-1111-2222');

INSERT INTO TBLMEMBER(MEMID, MEMPW, MEMNAME, MEMPHONE)
VALUES ('pbk', '1234', '병관쌤', '010-4611-2222');

INSERT INTO TBLMEMBER(MEMID, MEMPW, MEMNAME, MEMPHONE)
VALUES ('jjy', '1234', '준용대디', '010-3333-444');
