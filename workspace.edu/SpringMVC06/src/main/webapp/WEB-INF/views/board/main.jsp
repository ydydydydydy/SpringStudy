	  ...
	  <div class="panel panel-default">
	    ...
	    <div class="panel-body">
			<table id="boardList" class="table table-bordered table-hover">
				...
				<c:if test="${not empty mvo}"> <!-- 로그인 한 사람만 글쓰기 버튼이 보이도록 함 -->
				<tr>
					<td colspan="5">
	...
	<script type="text/javascript">
		...
		function makeView(data){
			...
			$.each(data, function(index, obj){
				...
				if("${mvo.member.memID}" == obj.memID){	// 로그인id = 작성자id일 때만 수정 삭제 버튼이 보이도록				
					listHtml += "<br>";
		...
		function goDelete(idx){
			$.ajax({
				url : "board/" + idx,
				type : "delete", // delete 방식은 post방식 중 하나(하위)
				beforeSend : function(xhr){
					xhr.setRequestHeader(csrfHeaderName, csrfTokenValue);
				},
				data : {"idx" : idx},
